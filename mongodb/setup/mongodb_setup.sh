...
gcloud compute ssh "$currentInstanceName" --zone "$zone" -- "sudo mongod --shardsvr --replSet shard${i}ReplSet --dbpath $data_dir --port 27018 --bind_ip 0.0.0.0 --auth --keyFile /etc/mongodb-keyfile --fork --logpath /var/log/mongodb/mongod.log"
